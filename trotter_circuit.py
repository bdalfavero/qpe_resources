...
from convert import cirq_pauli_sum_to_qiskit_pauli_op, qiskit_circuit_to_cirq
...
def first_order_trotter_for_paulisum(psum: cirq.PauliSum, dt: float, qs: List[cirq.Qid]) -> cirq.Circuit:
    ...
    groups = get_si_sets(psum, k=len(qs))
    return first_order_trotter_for_grouping(groups, dt, qs)
...
def cirq_paulihedral_circuit(ham: cirq.PauliSum, dt: float) -> cirq.Circuit:
    """Build a Paulihedral circuit and convert it to cirq."""

    ham_qiskit = cirq_pauli_sum_to_qiskit_pauli_op(ham)
    ev_gate = qiskit.circuit.library.PauliEvolutionGate(ham_qiskit, time=dt)
    ev_circuit = qiskit.QuantumCircuit(ham_qiskit.num_qubits)
    ev_circuit.append(ev_gate, range(ham_qiskit.num_qubits))
    cirq_circuit = qiskit_circuit_to_cirq(ev_circuit)
    return cirq_circuit
