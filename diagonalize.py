...
from kcommute import restrict_to
...
def get_stabilizer_matrix_from_paulis(stabilizers, qubits):
    ...
    for i, paulistring in enumerate(stabilizers):
        for key, value in paulistring.items():
            if value._name == "X":
                stabilizer_matrix[qubit_idx_map[key] + numq, i] = 1
            elif value._name == "Y":
                stabilizer_matrix[qubit_idx_map[key], i] = 1
                stabilizer_matrix[qubit_idx_map[key] + numq, i] = 1
            elif value._name == "Z":
                stabilizer_matrix[qubit_idx_map[key], i] = 1
...
def diagonalize_blockwise(
    paulis: List[cirq.PauliString], blocks: List[List[cirq.Qid]]
) -> Tuple[cirq.Circuit, List[cirq.PauliString]]:
    """Diagonalize each block individually."""

    block_circuits: List[cirq.Circuit] = []
    for block in blocks:
        paulis_restricted = [restrict_to(p, block) for p in paulis]
        diag_circuit, _ = diagonalize_pauli_strings(paulis_restricted, block)
        block_circuits.append(diag_circuit)
    
    total_circuit = cirq.Circuit()
    for ckt in block_circuits:
        total_circuit += ckt
    
    diag_paulis = []
    for pauli in paulis:
        conj_pauli = pauli.after(total_circuit)
        assert is_pauli_diagonal(conj_pauli), \
            f"Pauli string {conj_pauli} is not diagonal. Originally was {pauli}"
        diag_paulis.append(conj_pauli)
    
    return (total_circuit, diag_paulis)